...
let test_timeout = 60 (* seconds *)

let test_counts = ref (0,0) (* total, failed *)

let _ = Sys.set_signal Sys.sigalrm (Sys.Signal_handle(fun i -> raise (TestTimeout(test_timeout))))
...
let run_test (
  ...
  (*flush stdout;
  Unix.sleepf 0.5;*)
  (try let _ = Unix.alarm test_timeout in Ok(f input) with (TestTimeout(_) as tt) -> Error(tt) | ex -> Error(ex))
